...
def _normalize_db_url(url: str) -> str:
    # Render pode expor postgres://... ; SQLAlchemy 2 prefere postgresql+psycopg://...
...
# ===== Jinja: evitar erro no template (callable / now) =====
@app.context_processor
def inject_template_globals():
    return {"year": datetime.now(BRT).year}
...
class Cooperado(db.Model):
    ...
    # FOTO no banco:
    foto = db.Column(db.String(255))      # legado (se existir caminho)
    foto_data = db.Column(db.LargeBinary) # recomendado: binário da imagem
...
class Lancamento(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    data = db.Column(db.DateTime(timezone=True), nullable=False, default=utcnow)  # armazene UTC
...
class User(UserMixin, db.Model):
    __tablename__ = "users"
    id = db.Column(db.Integer, primary_key=True)
    email = db.Column(db.String(160), unique=True, nullable=False)  # pode ser "coopex"
    senha_hash = db.Column(db.String(255), nullable=False)
    # 'admin' | 'estabelecimento' | 'cooperado'
    tipo = db.Column(db.String(20), nullable=False, default="admin")
    # vínculo: p/ estabelecimento/cooperado aponta para id correspondente
...
def br_day_bounds_dual_utc_and_naive(d: date):
    s_aw, e_aw, s_nv, e_nv = br_day_bounds(d)
    return s_aw.astimezone(UTC), e_aw.astimezone(UTC), s_nv, e_nv

def br_month_bounds(dt_brt: datetime | None = None):
    now_brt = (dt_brt or datetime.now(BRT)).astimezone(BRT)
    first_aw = now_brt.replace(day=1, hour=0, minute=0, second=0, microsecond=0)
    if first_aw.month == 12:
        next_first_aw = first_aw.replace(year=first_aw.year + 1, month=1)
    else:
        next_first_aw = first_aw.replace(month=first_aw.month + 1)
    last_aw = next_first_aw - timedelta(microseconds=1)
    first_nv = first_aw.replace(tzinfo=None)
    last_nv = last_aw.replace(tzinfo=None)
    return first_aw, last_aw, first_nv, last_nv

def br_month_bounds_dual_utc_and_naive(dt_brt: datetime | None = None):
    f_aw, l_aw, f_nv, l_nv = br_month_bounds(dt_brt)
    return f_aw.astimezone(UTC), l_aw.astimezone(UTC), f_nv, l_nv

def to_brt_str(dt_aware: datetime) -> str:
    if dt_aware.tzinfo is None:
        dt_aware = dt_aware.replace(tzinfo=UTC)
    return dt_aware.astimezone(BRT).strftime("%d/%m/%Y %H:%M")

def to_brt_iso(dt_aware: datetime) -> str:
    if dt_aware.tzinfo is None:
        dt_aware = dt_aware.replace(tzinfo=UTC)
    return dt_aware.astimezone(BRT).strftime("%Y-%m-%dT%H:%M:%S")
...
def _ensure_db():
    """Garante tabelas e SEMPRE respeita senhas já existentes."""
    global DB_READY
    if DB_READY:
        return
    with app.app_context():
        ...
        admin_email = os.getenv("ADMIN_EMAIL", "coopex").strip().lower()
        admin_pass = os.getenv("ADMIN_PASSWORD", "coopex05289")
        if not User.query.filter_by(email=admin_email).first():
            admin = User(email=admin_email, tipo="admin")
...
def dashboard():
    ...
    # Top cooperados (mês atual BRT) cobrindo registros aware e naive
    start_utc, end_utc, start_nv, end_nv = br_month_bounds_dual_utc_and_naive()
    rows = (
        ...
        .filter(
            or_(
                ...
                and_(Lancamento.data >= start_nv,  Lancamento.data <= end_nv),
            )
        )
...
def listar_lancamentos():
    ...
    # Datas: sempre interpretar como BRT; cobrir registros aware (UTC) e naive (BRT)
    if data_inicio and data_fim:
        try:
            di = date.fromisoformat(data_inicio)
            df = date.fromisoformat(data_fim)
            di_utc, _, di_nv, _ = br_day_bounds_dual_utc_and_naive(di)
            _, df_utc, _, df_nv = br_day_bounds_dual_utc_and_naive(df)
            q = q.filter(
                or_(
                    and_(Lancamento.data >= di_utc, Lancamento.data <= df_utc),
                    and_(Lancamento.data >= di_nv,  Lancamento.data <= df_nv),
                )
            )
        except Exception:
            pass
    else:
        ms_utc, me_utc, ms_nv, me_nv = br_month_bounds_dual_utc_and_naive()
        q = q.filter(
            or_(
                and_(Lancamento.data >= ms_utc, Lancamento.data <= me_utc),
                and_(Lancamento.data >= ms_nv,  Lancamento.data <= me_nv),
            )
        )
...
def exportar_lancamentos():
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        pd.DataFrame(data).to_excel(writer, index=False, sheet_name="Lancamentos")
...
@login_required
def api_ultimo_lancamento_hifen():
    last = db.session.query(func.max(Lancamento.id)).scalar() or 0
    return jsonify({"last_id": int(last)})

@app.route("/api/ultimo_lancamento")
@login_required
def api_ultimo_lancamento_underscore():
    # Alias para compatibilidade com template que usa underscore
    return api_ultimo_lancamento_hifen()
...
def initdb():
    """Cria tabelas e admin padrão (ADMIN_EMAIL/ADMIN_PASSWORD ou 'coopex'/'coopex05289')."""
    db.create_all()
    ...
    if not Estabelecimento.query.first():
        db.session.add(Estabelecimento(nome="Loja Exemplo", username="loja"))

    if not Cooperado.query.first():
        db.session.add(Cooperado(nome="Maria Silva", username="maria", credito=1000))
